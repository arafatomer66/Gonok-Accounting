...
@Component({
  selector: 'gonok-payment-in',
  standalone: true,
  imports: [TransactionListComponent, TransactionFormComponent, ConfirmDialogComponent],
  template: `
    <div class="page-header">
      <h1 class="page-header__title">Payment In</h1>
      <button class="btn btn--primary" (click)="openForm()">+ New Payment In</button>
    </div>

    <gonok-transaction-list
      [transactions]="transactionStore.paymentsIn()"
      partyLabel="Customer"
      invoiceLabel="Receipt No"
      emptyMessage="No payment in transactions yet."
      [hidedue]="true"
      (edit)="editTx($event)"
...
export class PaymentInComponent implements OnInit {
  ...
  txType = ETransactionType.PAYMENT_IN;
