...
@Component({
  selector: 'gonok-purchase-return',
  standalone: true,
  imports: [TransactionListComponent, TransactionFormComponent, ConfirmDialogComponent],
  template: `
    <div class="page-header">
      <h1 class="page-header__title">Purchase Returns</h1>
      <button class="btn btn--primary" (click)="openForm()">+ New Purchase Return</button>
    </div>

    <gonok-transaction-list
      [transactions]="transactionStore.purchaseReturns()"
      partyLabel="Supplier"
      invoiceLabel="Return No"
      emptyMessage="No purchase return transactions yet."
      (edit)="editTx($event)"
    ...
    <gonok-confirm-dialog
      ...
      message="Delete this purchase return? Stock and balance will be reversed."
      confirmLabel="Delete"
      variant="danger"
      (confirmed)="doDelete()"
...
export class PurchaseReturnComponent implements OnInit {
  ...
  txType = ETransactionType.PURCHASE_RETURN;
