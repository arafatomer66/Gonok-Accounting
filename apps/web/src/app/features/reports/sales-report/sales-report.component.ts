  ...
  template: `
    ...
    <div class="table-wrapper">
      <table class="table">
        ...
        <tbody>
          @for (tx of filteredSales(); track tx.uuid) {
            <tr>
            ...
            <tr>
              <td colspan="7" class="text-center text-muted">No sales found for the selected filters.</td>
            </tr>
          }
        </tbody>
      </table>
...
export class SalesReportComponent implements OnInit {
  ...
  filteredSales = computed(() => {
    let sales = this.transactionStore.sales();
    ...
    if (party) {
      sales = sales.filter((t) => t.party_uuid === party);
    }
    return [...sales].sort(
  ...
  totalAmount = computed(() =>
    this.filteredSales().reduce((s, t) => s + (t.total_amount || 0), 0),
  );
  totalPaid = computed(() =>
    this.filteredSales().reduce((s, t) => s + (t.paid_amount || 0), 0),
  );
  totalDue = computed(() =>
    this.filteredSales().reduce((s, t) => s + (t.due_amount || 0), 0),
  );
