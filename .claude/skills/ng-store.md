---
description: Create or extend an @ngrx/signals store for this project.
---
...
10. Include a `reset()` method and register it in `business-switch.service.ts`
...
async addEntity(data: Partial<IEntity>): Promise<IEntity> {
  ...
  const entity: IEntity = {
    ...
    business_uuid: bizUuid, branch_uuid: null,
    ...data,
...
async updateEntity(uuid: string, data: Partial<IEntity>): Promise<void> {
  const existing = store.entities().find(e => e.uuid === uuid);
  if (!existing) return;
...
- Register `reset()` in `apps/web/src/app/core/services/business-switch.service.ts`
